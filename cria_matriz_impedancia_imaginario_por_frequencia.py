...
arquivos_v_imaginario = procura("imaginario_v_node*.csv", "./arquivos_csv_base/")
...
for harmonico in lista_de_harmonicos_reduzido:
    ...
    for i in range(len(lista_de_nos)):
        ...
        for arquivo in arquivos_v_imaginario:
            ...
            if final_nome_arquivo_sem_extensao == node:
                ...
                arquivo_df = pd.read_csv(caminho, index_col=0).drop([1,3,5])
    ...
    harmonico_df.to_csv("Z_imaginario_harmonico_{}.csv".format(harmonico))
    indice = indice + 1

print("Matrizes Z imaginários criadas")
